...
import db as novalib_db
import app as novalib_app


@pytest.fixture
def client():
    """Create a fresh test client with a clean database for each test."""
    # Wipe and recreate all tables so each test starts clean
    conn = novalib_db.get_db()
    cur  = conn.cursor()
    for table in ("completed_stories", "user_achievements", "user_stats", "users"):
        try:
...
def test_register_success(client):
    ...
    res  = client.post('/api/register',
                       data=json.dumps({'username': 'testuser', 'password': 'secret123', 'email': 'test@test.com'}),
...
def test_register_duplicate_username(client):
    ...
    client.post('/api/register',
                ...
                content_type='application/json')
    res  = client.post('/api/register',
                       data=json.dumps({'username': 'testuser', 'password': 'different', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
...
def test_register_short_password(client):
    ...
    res  = client.post('/api/register',
                       data=json.dumps({'username': 'testuser', 'password': '123', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
    assert res.status_code == 400
    assert 'error' in data
...
def test_register_empty_username(client):
    ...
    res  = client.post('/api/register',
                       data=json.dumps({'username': '', 'password': 'secret123', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
    assert res.status_code == 400
    assert 'error' in data
...
def test_register_password_with_spaces(client):
    ...
    res  = client.post('/api/register',
                       data=json.dumps({'username': 'testuser', 'password': 'hello world', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
    assert res.status_code == 200
    assert data['username'] == 'testuser'
...
def test_username_with_numbers(client):
    ...
    res  = client.post('/api/register',
                       data=json.dumps({'username': 'user123', 'password': 'secret123', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
    assert res.status_code == 200
...
def test_login_wrong_password(client):
    ...
    client.post('/api/register',
                ...
                content_type='application/json')
    res  = client.post('/api/login',
                       data=json.dumps({'username': 'testuser', 'password': 'wrongpassword', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
...
def test_login_nonexistent_user(client):
    ...
    res  = client.post('/api/login',
                       data=json.dumps({'username': 'nobody', 'password': 'secret123', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
    assert res.status_code == 401
    assert 'error' in data
...
def test_login_empty_fields(client):
    ...
    res  = client.post('/api/login',
                       data=json.dumps({'username': '', 'password': '', 'email': 'test@test.com'}),
                       content_type='application/json')
    data = json.loads(res.data)
    assert res.status_code == 400
    assert 'error' in data
